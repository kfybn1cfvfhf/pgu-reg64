...
abstract public class AbstractServiceOrderControllerEDS24 <F extends AbstractServiceOrderForm> extends com.nvision.pgu.core.controllers.AbstractServiceOrderControllerEDS24<F>{
    ...
    protected Map<String, Object> formToMap(F form) {
        ...
        while(tmpFileBeanMapIterator.hasNext()) {
            ...
            // Формирование полей файла вручную (отключено для контроллера с подписью)
    ...
    protected List<String> getBlockFileList(F form) {
        Map<String, FileBean> tmpFileBeanMap = form.getFileBlock();
        Iterator tmpFileBeanMapIterator = tmpFileBeanMap.entrySet().iterator();
        ArrayList<String> tmpList = new ArrayList<String>();
        while(tmpFileBeanMapIterator.hasNext()) {
            Map.Entry pairs = (Map.Entry)tmpFileBeanMapIterator.next();
            FileBean tmpRes = (FileBean) pairs.getValue();
            tmpList.add(tmpRes.getFileID());
        }
        return tmpList;
    }
